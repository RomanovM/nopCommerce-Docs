...
- **Limited to stores**, if the product is sold in specific stores. Leave the field empty in case this functionality is not required. Note that in order to use this functionality you have to disable the following setting: **Configuration → Catalog Settings → Ignore** **“Limit per store” rules **(sitewide). Learn how to set up several stores [here](xref:ru-RU/user-guide/configuring/settingup/mainstore/multiple-store).
- **Vendor**. You can manage [vendors](xref:ru-RU/user-guide/configuring/settingup/customers/vendors/index) in **Customers → Vendors**
